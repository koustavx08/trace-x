...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("open", "in_progress", "closed", "archived", name="casestatus"),
            default="open",
            nullable=False,
        ),
        sa.Column(
            "assigned_to", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=True, index=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "attribution_status",
            sa.Enum(
                "unverified", "under_review", "attributed", "confirmed", name="attributionstatus"
            ),
            default="unverified",
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            UUID(as_uuid=True),
            sa.ForeignKey("investigation_runs.id"),
            nullable=True,
        ),
        sa.Column("title", sa.String(255), nullable=False),
    ...
    op.create_index(
        "ix_investigation_runs_case_status", "investigation_runs", ["case_id", "status"]
    )
    op.create_index(
        "ix_investigation_runs_wallet_status", "investigation_runs", ["wallet_id", "status"]
    )
