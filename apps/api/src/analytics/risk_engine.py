...
from ..graph.models import ConfidenceLevel, EntityType, GraphWallet, GraphTransaction
...
class RiskScoringEngine:
    ...
    async def _check_rapid_movement(self, transactions: List[GraphTransaction]) -> List[RiskFactor]:
        ...
        if len(rapid_sequences) >= 3:
            factors.append(RiskFactor(
                ...
                severity=RiskSeverity.HIGH if len(rapid_sequences) >= 5 else RiskSeverity.MEDIUM,
        ...
        if entity_type in high_risk_types:
            ...
            factors.append(RiskFactor(
                ...
                evidence={
                    ...
                    "confidence": confidence.value if confidence else ConfidenceLevel.UNKNOWN.value,
                },
    ...
    async def _check_large_transfers(self, transactions: List[GraphTransaction]) -> List[RiskFactor]:
        factors = []
    ...
    async def _check_cross_chain_bridges(self, transactions: List[GraphTransaction]) -> List[RiskFactor]:
        factors = []

        bridge_txs = [
            tx for tx in transactions
            if tx.token_symbol and tx.token_symbol.upper() in ["USDC", "USDT", "WETH", "MATIC"]
            and tx.method and "bridge" in tx.method.lower()
        ]
        ...
        if bridge_txs:
            factors.append(RiskFactor(
                ...
                evidence={
                    ...
                    "tokens": list(set(tx.token_symbol for tx in bridge_txs if tx.token_symbol)),
    ...
    async def _check_contract_interactions(self, transactions: List[GraphTransaction]) -> List[RiskFactor]:
        factors = []

        contract_txs = [tx for tx in transactions if tx.method and tx.method != "transfer"]
        if len(contract_txs) > 10:
            unique_methods = len(set(tx.method for tx in contract_txs if tx.method))
            ...
            factors.append(RiskFactor(
                ...
                evidence={
                    ...
                    "methods": list(set(tx.method for tx in contract_txs if tx.method))[:10],
                },
                confidence=ConfidenceLevel.PROBABLE,
            ))
    ...
    def _generate_summary(self, factors: List[RiskFactor], score: float, level: RiskSeverity) -> str:
        if not factors:
        ...
        if critical:
            parts.append(f"{len(critical)} critical factor(s): {', '.join(f.factor_type.value.replace('_', ' ') for f in critical)}.")
        if high:
            parts.append(f"{len(high)} high-risk factor(s): {', '.join(f.factor_type.value.replace('_', ' ') for f in high)}.")
        if medium:
            parts.append(f"{len(medium)} medium-risk factor(s): {', '.join(f.factor_type.value.replace('_', ' ') for f in medium)}.")
