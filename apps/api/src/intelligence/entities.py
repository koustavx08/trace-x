...
class EntityIntelligence:
    ...
    async def _load_major_exchanges(self) -> List[EntityRecord]:
        exchanges = [
            {"name": "Binance", "addresses": {
                1: ["0x28C6c06298d514Db089934071355E5743bf21d60", "0x21a31Ee1afC51d94C2eF3CAaDB94D5c86C1F9e3d"],
                137: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.CONFIRMED},
            {"name": "Coinbase", "addresses": {
                1: ["0xA9d1e08C7793af67e9d92fe308d5697FB81d3E43", "0x503828976D22510aad0201ac7EC88293211D23Da"],
            }, "confidence": ConfidenceLevel.CONFIRMED},
            {"name": "Kraken", "addresses": {
                1: ["0x2910543Af39abA0Cd09dBb2D50200b3E800A63D2", "0x0A869d79a7052c7f1b55a8EBabAa43105310D4D2"],
            }, "confidence": ConfidenceLevel.CONFIRMED},
            {"name": "OKX", "addresses": {
            ...
            {"name": "Huobi", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.HIGH_CONFIDENCE},
            {"name": "KuCoin", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.HIGH_CONFIDENCE},
            {"name": "Gate.io", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
    ...
    async def _load_known_mixers(self) -> List[EntityRecord]:
        mixers = [
            {"name": "Tornado Cash", "addresses": {
                1: ["0x722122dF12D4e14e13Ac3b6895a86e84145b6967", "0x169455c72558a2D9A2C4a5b8F6e9e8D7a6B5c4D3"],
            }, "confidence": ConfidenceLevel.CONFIRMED},
            {"name": "Wasabi Wallet", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.PROBABLE},
            {"name": "Samourai Whirlpool", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.PROBABLE},
            {"name": "Mixer", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.PROBABLE},
        ]
    ...
    async def _load_defi_protocols(self) -> List[EntityRecord]:
        defi = [
            {"name": "Uniswap V3", "addresses": {
                1: ["0x1F98431c8aD98523631AE4a59f267346ea31F984"],
                137: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.CONFIRMED},
            {"name": "Aave V3", "addresses": {
                1: ["0x87870B93F8aD8E5F8a8A8A8A8A8A8A8A8A8A8A8A"],
            }, "confidence": ConfidenceLevel.HIGH_CONFIDENCE},
            {"name": "Curve Finance", "addresses": {
                1: ["0x0000000000000000000000000000000000001010"],
            }, "confidence": ConfidenceLevel.HIGH_CONFIDENCE},
        ]
    ...
    async def lookup_entity(self, address: str, chain: str) -> Optional[GraphEntity]:
        from ..graph.client import Neo4jClient
        query = """
        MATCH (e:Entity {address: $address, chain: $chain})
        RETURN e
        """
        result = await Neo4jClient.execute_query(query, {"address": address.lower(), "chain": chain})
        if not result:
            return None
        e = result[0]["e"]
        return GraphEntity(
    ...
    async def enrich_wallet(self, wallet_address: str, chain: str) -> Optional[GraphEntity]:
